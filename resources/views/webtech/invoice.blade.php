                        ...
                        <div class="text-end">
                            <form method="post" action="{{ url('send-email-pdf') }}" enctype="multipart/form-data">
                                ...
                                <button class="btn btn-success" onclick="printme('invoic')"><i class="fa fa-print"></i>
                                    Print</button>
                                <button type="button" class="btn btn-danger" id="download"><i class="fa fa-file-pdf-o"></i>
                                        ...
                                        <tbody>
                                            @foreach ($data->compservice as $key => $invoice)
                                                <tr id="{{ $invoice->service->parent->stype_name }}">
                                                    <td> <button class="btn btn-sm btn-secondary hide"
                                                            data-html2canvas-ignore="true"
                                                            data-serviceName="{{ $invoice->service->parent->stype_name }}"
                                                            onclick="checkMe(this)">-</button></td>
                                                    <td colspan="3">{{ $invoice->service->parent->stype_name }}</td>
                                                    <input type="hidden" name="particular[]"
                                                        value="{{ $invoice->service->parent->stype_name }}">
                                                    <td class="amount text-center"
                                                        value="{{ $invoice->amountafterdiscount }}">
                                                        {{ $invoice->amountafterdiscount }}</td>
